...
export const getAllJobs = async (req, res) => {
  try {
    ...
    const jobs = await Job.find(query)
      .populate(
        "companyId",
        "profile.companyName profile.industry profile.website email"
      )
      .select("-applicants")
      .sort({ createdAt: -1 });
  ...
  try {
    ...
    const applicants = jobs.map((job) => ({
      ...
      applicants: job.applicants.map((a) => ({
        ...
        phoneNumber: a.studentId.profile.phoneNumber,
        skills: a.studentId.profile.skills,
        location: a.studentId.profile.location,
        education: a.studentId.profile.education,
        status: a.status,
      })),
    }));
